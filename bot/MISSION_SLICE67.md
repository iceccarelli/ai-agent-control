MISSION — SLICE 67: FORWARD SHADOW ON GROWN POST-t1 WINDOW (5 DAYS)
...
3. Human extended real BTCUSDT linear 1d + funding.
   docs/human/HUMAN_DATA_NOTE_SLICE67.md is prose only.
   Verify via FILE sha + last dates + after_t1 count.
   Expect after_t1_linear >= 5 including:
     2026-08-10, 2026-08-11, 2026-08-12, 2026-08-13, 2026-08-14
   linear rows expected 1466; no open 2026-08-15 bar.
...
5. Labels:
   ...
   Report delta vs slice 66 (window grew?).
   Ceiling from bar count; observed <= ceiling.
   ~6 closed forward bars needed before one trade can complete.
   With 5 days, ceiling still 0 — forward_n_trades may still be 0 — VALID.
...
shipping from slice-61 without restore if apparatus missing.
...
2 artifacts/slice67_data_freshness.json — FILES only; delta vs 66;
  prefix-append integrity; bars_fabricated=0; BTC-only manifest discipline.
3 artifacts/slice67_forward_shadow.json — frozen pack; forward after t1;
...
6 Verdict:
   SLICE67_VERDICT / extension_present / after_t1_linear / dates /
   new_linear_bars_since_slice66 / the_window_grew /
