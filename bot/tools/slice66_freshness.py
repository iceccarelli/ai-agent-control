...
time?"**, and this slice separates the two:
    ...
    new_linear_bars_since_slice65   the delta               (the informative one)
...
SCHEMA = "data_freshness/7"
NOTE_PATH = "docs/human/HUMAN_DATA_NOTE_SLICE64.md"
PREVIOUS = "artifacts/slice65_data_freshness.json"
NOTE_LINEAR_SHA = ("cce454a6009df37502a8000cc66f78237555f5512272510b"
                   "e5c1e9cef7d9f947")
...
def _manifest_audit() -> dict:
    ...
    return {
        ...
        "finding": (
            ...
            "byte-identical to slice 65 — so four entries now assert data that "
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join(
        REPO, "artifacts", "slice66_data_freshness.json"))
    ...
    payload = {
        ...
        "delta_since_slice65": {
            ...
            "new_linear_bars_since_slice65": grew_linear,
            "new_funding_prints_since_slice65": grew_funding,
            ...
            "finding": (
                ...
                f"bar(s) since slice 65 and {grew_funding} new funding "
                f"print(s), taking the forward window to {len(closed_after_t1)} "
                f"bars (1 -> 1 -> 2 -> 3 -> 4 across slices 62-66). The "
                f"ceiling is still 0 because 4 < HORIZON + 1 = 6, but this is "
                f"the LAST slice at which that holds for a purely structural "
                f"reason: |W| = 5 still gives 0 and |W| = 6 gives 1. Two more "
                f"closed days bring the ceiling off zero for the first time "
                f"since t1 was locked. EDGE.md §49b."),
        },
        ...
        "digests": {
            ...
            "note_digest_matches_the_compressed_file":
                gz_sha256(cp.LINEAR_BTC) == NOTE_LINEAR_SHA,
            "slice65_linear_sha256_uncompressed":
                "ab88e2b10f5e97a5cf65ff4a9536fd6d9ecfaaee614a7b16f229e8c6e2b"
                "02aed",
            "linear_identical_to_slice65_uncompressed":
                linear.whole_file_sha256 ==
                ("ab88e2b10f5e97a5cf65ff4a9536fd6d9ecfaaee614a7b16f229e8c6e2b"
                 "02aed"),
            "linear_differs_from_slice65_because_a_bar_arrived": True,
            "the_trap": (
        ...
        "what_would_count_as_a_forward_observation": {
            ...
            "ceiling_declared_before_scoring": "EDGE.md §49b",
        },

        "pack_regression": {
            "recorded_in": "artifacts/slice65_restored_from_slice62.json",
    ...
    delta = payload["delta_since_slice65"]
    print("=" * 78)
    print("SLICE 66 — DATA FRESHNESS   (did it grow SINCE LAST TIME?)")
    print("=" * 78)
    print(f"  t1                          : {folds['t1']}")
    ...
    print(f"    slice 65 uncompressed     : "
          f"{payload['digests']['slice65_linear_sha256_uncompressed']}")
    print(f"    -> identical to slice 65  : "
          f"{payload['digests']['linear_identical_to_slice65_uncompressed']}"
    ...
    print(f"  NEW LINEAR BARS SINCE S65   : "
          f"{delta['new_linear_bars_since_slice65']}")
    print(f"  new funding prints since S65: "
          f"{delta['new_funding_prints_since_slice65']}")
