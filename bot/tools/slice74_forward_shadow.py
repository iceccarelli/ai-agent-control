...
SLICE 74 — WHAT CHANGED IN THIS FILE, AND WHAT DID NOT
======================================================
...
slice needs to be honest (`window_grew_since_slice73`). `diff` the two files.
...
SCHEMA = "forward_shadow_observation/13"
SLICE = 74
...
def build(*, data_dir: str, funding_path: str, warmup: int,
    ...
    return {
        ...
        "ceiling": {
            ...
            "declared_in": "EDGE.md §57a, before this tool ran",
            "observed_forward_closed_trades": forward_n_trades,
            "within_ceiling": forward_n_trades <= ceiling,
            "note_on_this_milestone": (
                "Ceiling 7 at N=12: not 6 — that was slice 73, at N=11 — and "
                "not 8, which would need N=13. What moved this slice is "
                "again upstream: a SECOND setup, 2026-08-21, whose 16:00 "
                "print stands at the bar's close. It is the last bar, so it "
                "is not a flag — the same structure 2026-08-19 was in one "
                "slice ago. 2026-08-19 is still flagged and still not "
                "scoreable: eligibility reaches back seven bars and the "
                "corpus must reach 2026-08-26. Four days short, by "
                "subtraction rather than forecast. EDGE.md §57b, §56d."),
            ...
            "if_exceeded": (
                "CORRECTED IN SLICE 74 (EDGE.md §55c). Every artefact from "
        ...
        "why_neither_moved": (
            ...
            "at 08:00 that day is not the decision rate. §57c adds the "
        ...
        "window_grew_since_slice73": _window_grew(
            len(closed_forward),
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join(
        REPO, "artifacts", "slice74_forward_shadow.json"))
    parser.add_argument("--log", default=os.path.join(
        REPO, "artifacts", "slice74_forward_shadow.log"))
    ...
    lines = [
        "=" * 78,
        "SLICE 74 — FORWARD SHADOW SEGMENT   (post-t1 bars only)",
        "=" * 78,
        "",
