...
SLICE 69 — WHAT CHANGED IN THIS FILE, AND WHAT DID NOT
======================================================
...
slice needs to be honest (`window_grew_since_slice68`). `diff` the two files.
...
SCHEMA = "forward_shadow_observation/9"
...
def _window_grew(closed_forward_bars: int) -> dict:
    ...
    return {
        ...
        "note": (
            "THE WINDOW REACHED SEVEN BARS AND THE CEILING IS 2 — two forward "
            "decision bars could now yield a trade closing inside the window. "
            "ZERO SETUPS FIRED on all seven: every rate at a decision is below "
            "FUND_ABS. The measurement is 'ceiling 2, setups 0 of 7, fills 0' "
            "and no second sentence is entitled to more. It says the funding "
            "regime from 2026-08-10 to 2026-08-16 never stood at or above "
            "FUND_ABS at a decision; it says NOTHING about whether the rule "
            "makes money, because no position was opened and only selectivity "
            "was exercised, never skill. The cleared run scheduled 41 trades "
            "in about two years, roughly one per eighteen days, so seven quiet "
            "days is unremarkable under the null and the alternative alike and "
            "would remain so at fourteen. EDGE.md §52b, §52c, §52d."),
    }
...
def build(*, data_dir: str, funding_path: str, warmup: int,
    ...
    return {
        ...
        "join_rule": ("last funding print at-or-before the bar's CLOSE — "
                      ...
                      "backwards in time"),
        "why_neither_moved": (
            "The 2026-08-12T00:00Z print equalled FUND_ABS exactly and was "
            "superseded before that bar's close (EDGE.md §51c). Adjusting "
            "either the threshold or the join to capture it would be fitting "
            "the rule to the data AFTER seeing which prints missed — and the "
            "adjustment that would rescue that bar is precisely the one that "
            "would void the clear it was measured under."),
        ...
        "window_grew_since_slice68": _window_grew(len(closed_forward)),
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join(
        REPO, "artifacts", "slice69_forward_shadow.json"))
    parser.add_argument("--log", default=os.path.join(
        REPO, "artifacts", "slice69_forward_shadow.log"))
    ...
    lines = [
        "=" * 78,
        "SLICE 69 — FORWARD SHADOW SEGMENT   (post-t1 bars only)",
        "=" * 78,
        "",
