...
WHY THIS RUN IS DIFFERENT FROM SLICE 62's
=========================================
It is not, and that is the finding. Slice 62 re-evaluated this gate against a
corpus that had just gained its first post-`t1` bar; `forward_shadow_clean` read
0 / 20 because one bar cannot produce a closed trade under a five-bar horizon.

**The corpus has not gained a decision bar since.** The linear file is
byte-identical to slice 62's (§46c); a single funding print arrived, and the
decision clock is the daily bar. So the item reads 0 / 20 for the same reason,
over the same window, and this artefact says so explicitly rather than
presenting an unchanged number as a fresh re-evaluation.
...
PREVIOUS = "artifacts/slice62_promotion_gate.json"
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join(
        REPO, "artifacts", "slice63_promotion_gate.json"))
    ...
    # owned by observation, and this slice observed the same window slice 62
    # observed. Nothing about it moved, and the evidence string says which of
    # the two possible reasons for an unchanged count holds.
    checklist["forward_shadow_clean"].update({
        ...
        "evidence": (
            ...
            f"trades; 0 of {pg.MIN_FORWARD_DAYS} days. UNCHANGED FROM SLICE "
            f"62, AND FOR AN UNCHANGED REASON: the forward window did not "
            f"grow. The linear corpus is byte-identical to slice 62's "
            f"(uncompressed sha256 32ca5971...), so {closed_bars} closed "
            f"linear daily bar(s) lie strictly after t1 — the same bar. One "
            f"funding print arrived, and the decision clock is the daily bar; "
            f"a funding print without a bar to decide is not an observation. "
            f"With a 5-bar horizon and next-open entry, 6 closed forward bars "
            f"must exist before the first forward trade can close. The rule "
    ...
    payload = {
        ...
        "doc": previous["doc"],
        "design_note": "EDGE.md §46d (the forward rule and its ceiling, restated)",
        "default": "REFUSE",
        ...
        "items_complete_unchanged_from_slice_62":
    ...
    print("=" * 78)
    print("SLICE 63 — PROMOTION GATE   (re-evaluated against REAL forward data)")
    print("=" * 78)
    ...
    print(f"  items complete               : {payload['items_complete']} / "
          f"{payload['items_total']}   (unchanged from slice 62: "
          f"{payload['items_complete_unchanged_from_slice_62']})")
