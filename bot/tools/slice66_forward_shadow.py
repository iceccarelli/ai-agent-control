...
SLICE 66 — WHAT CHANGED IN THIS FILE, AND WHAT DID NOT
======================================================
...
slice needs to be honest (`window_grew_since_slice65`). `diff` the two files.
...
SCHEMA = "forward_shadow_observation/6"
...
def _window_grew(closed_forward_bars: int) -> dict:
    ...
    return {
        "comparable": True,
        "previous_artefact": "artifacts/slice65_forward_shadow.json",
        ...
        "note": (
            "THE WINDOW GREW AGAIN — 3 bars to 4. The ceiling is still zero "
            "because max(0, 4 - HORIZON) = 0, and this is the LAST slice at "
            "which zero follows purely from the window being shorter than the "
            "horizon: |W| = 5 still yields 0 (the sixth bar a trade needs does "
            "not exist) and |W| = 6 yields 1. TWO MORE CLOSED DAYS bring the "
            "ceiling off zero for the first time since t1 was locked. That is "
            "a prediction recorded before the fact, checkable in slices 67-68. "
            "Five slices have now reported forward_n_trades = 0 and every one "
            "ran at ceiling 0, so none of them is evidence about the edge. "
            "From |W| = 6 onward a zero WOULD carry information. EDGE.md "
            "§49b, §49c."),
    }
...
def build(*, data_dir: str, funding_path: str, warmup: int,
    ...
    return {
        ...
        "ceiling": {
            ...
            "declared_in": "EDGE.md §49b, before this tool ran",
        ...
        "window_grew_since_slice65": _window_grew(len(closed_forward)),
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join(
        REPO, "artifacts", "slice66_forward_shadow.json"))
    parser.add_argument("--log", default=os.path.join(
        REPO, "artifacts", "slice66_forward_shadow.log"))
    ...
    lines = [
        "=" * 78,
        "SLICE 66 — FORWARD SHADOW SEGMENT   (post-t1 bars only)",
        "=" * 78,
        "",
