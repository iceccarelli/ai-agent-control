...
time?"**, and this slice separates the two:
    ...
    new_linear_bars_since_slice68   the delta               (the informative one)
...
PREVIOUS = "artifacts/slice68_data_freshness.json"
NOTE_LINEAR_SHA = ("d06fcd33020e9643ca8adfe66b6a5295a184c6cb537bce06"
                   "29e21e619d036c52")
...
def _manifest_audit() -> dict:
    ...
    return {
        ...
        "finding": (
            ...
            "byte-identical to slice 68 — so four entries now assert data that "
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join(
        REPO, "artifacts", "slice69_data_freshness.json"))
    ...
    _claims = {
        "closed bars after t1 = 7": len(closed_after_t1) == 7,
        "after_t1_dates": [c["time_period_start"][:10]
                           for c in closed_after_t1]
        == ["2026-08-10", "2026-08-11", "2026-08-12", "2026-08-13",
            "2026-08-14", "2026-08-15", "2026-08-16"],
        "linear rows = 1468": linear.rows_on_disk == 1468,
        "linear sha256 (compressed)":
            gz_sha256(cp.LINEAR_BTC) == NOTE_LINEAR_SHA,
        "append-only; no pre-t1 rewrite": all_append_only,
        "grew vs slice 69 (6 -> 7)": grew_linear == 1,
        "08-17 NOT appended": not any(
            row["time_period_start"].startswith("2026-08-17")
            for row in cp.read_rows(cp.LINEAR_BTC)),
        "ceiling: max(0, 7-5) = 2":
            max(0, len(closed_after_t1) - fb.HORIZON) == 2,
        "synthetic false": all(
        ...
        "pack base: tradingbot_slice68.zip": _pack_base_claim_is_true(),
    }
    ...
    payload = {
        ...
        "delta_since_slice68": {
            ...
            "new_linear_bars_since_slice68": grew_linear,
            "new_funding_prints_since_slice68": grew_funding,
            ...
            "why_this_field_exists": (
                ...
                "EDGE.md §46c."),
            "finding": (
                f"THE WINDOW REACHED SEVEN BARS. {grew_linear} new closed "
                f"daily bar(s) since slice 68, taking the ceiling to "
                f"{max(0, len(closed_after_t1) - fb.HORIZON)}. Two forward "
                f"decision bars could now yield a trade that closes inside the "
                f"window. Zero setups fired: every rate at a decision is below "
                f"FUND_ABS. The measurement is 'ceiling 2, setups 0 of 7, "
                f"fills 0' and there is no second sentence entitled to more. "
                f"EDGE.md §52b, §52c, §52d."),
        },
        ...
        "digests": {
            ...
            "note_digest_matches_the_compressed_file":
                gz_sha256(cp.LINEAR_BTC) == NOTE_LINEAR_SHA,
            "slice68_linear_sha256_uncompressed":
                "6403f090a8375fc8a293c871615c270ca606c526bad3929e905d39892c"
                "8ea023",
            "linear_identical_to_slice68_uncompressed":
                linear.whole_file_sha256 ==
                ("6403f090a8375fc8a293c871615c270ca606c526bad3929e905d39892c"
                 "8ea023"),
            "linear_differs_from_slice68_because_a_bar_arrived": True,
            "the_trap": (
        ...
        "claim_vs_files_discrepancy": not _claims["grew vs slice 69 (6 -> 7)"],
        "growth_claim_verified": _claims["grew vs slice 69 (6 -> 7)"],
        ...
        "pack_base_claim": {
            # prior-slice: "slice-61" names the wrong parent this claim is
            # about, and never advances.
            "claim": "Pack base: post-restore slice68 tree (not slice-61)",
            ...
            "evidence": "artifacts/slice69_restored_from_slice68.json",
        ...
        "what_would_count_as_a_forward_observation": {
            ...
            "ceiling_declared_before_scoring": "EDGE.md §52b",
        },

        "pack_regression": {
            "occurred_this_slice": False,
            "pack_base": "the slice-68 deliverable, verified by digest",
            ...
            "summary": ("No regression. The pack is the previous deliverable, "
                        ...
                        "exists — the second consecutive slice with no "
                        "restoration tax. The five-slice run of slice-61 "
                        # prior-slice: the run of regressions was slices 63-67.
    ...
    delta = payload["delta_since_slice68"]
    print("=" * 78)
    print("SLICE 69 — DATA FRESHNESS   (did it grow SINCE LAST TIME?)")
    print("=" * 78)
    print(f"  t1                          : {folds['t1']}")
    ...
    print(f"    slice 68 uncompressed     : "
          f"{payload['digests']['slice68_linear_sha256_uncompressed']}")
    print(f"    -> identical to slice 68  : "
          f"{payload['digests']['linear_identical_to_slice68_uncompressed']}"
    ...
    print(f"  NEW LINEAR BARS SINCE S68   : "
          f"{delta['new_linear_bars_since_slice68']}")
    print(f"  new funding prints since S68: "
          f"{delta['new_funding_prints_since_slice68']}")
