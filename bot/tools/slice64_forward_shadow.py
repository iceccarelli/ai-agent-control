...
SLICE 64 — WHAT CHANGED IN THIS FILE, AND WHAT DID NOT
======================================================
...
slice needs to be honest (`window_grew_since_slice63`). `diff` the two files.
...
SCHEMA = "forward_shadow_observation/4"
...
def _window_grew(closed_forward_bars: int) -> dict:
    ...
    return {
        "comparable": True,
        "previous_artefact": "artifacts/slice63_forward_shadow.json",
        ...
        "note": (
            "THE WINDOW GREW — the first genuine growth since t1 was locked; "
            "slices 62 and 63 both scored a one-bar window. The ceiling did "
            "not move with it: max(0, N - HORIZON) is still zero at two bars. "
            "A larger window that still cannot produce a closed trade is "
            "progress in the pilot and no evidence at all, and those are "
            "different sentences. EDGE.md §47a, §47d."),
    }
...
def build(*, data_dir: str, funding_path: str, warmup: int,
    ...
    return {
        ...
        "window_grew_since_slice63": _window_grew(len(closed_forward)),
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join(
        REPO, "artifacts", "slice64_forward_shadow.json"))
    parser.add_argument("--log", default=os.path.join(
        REPO, "artifacts", "slice64_forward_shadow.log"))
    ...
    lines = [
        "=" * 78,
        "SLICE 64 — FORWARD SHADOW SEGMENT   (post-t1 bars only)",
        "=" * 78,
        "",
