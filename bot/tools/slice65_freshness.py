...
time?"**, and this slice separates the two:
    ...
    new_linear_bars_since_slice64   the delta               (the informative one)
...
SCHEMA = "data_freshness/6"
NOTE_PATH = "docs/human/HUMAN_DATA_NOTE_SLICE64.md"
PREVIOUS = "artifacts/slice64_data_freshness.json"
NOTE_LINEAR_SHA = ("39dcd4687d837a84e74db0612fed1dcbdabc3a7a4d3e8a015"
                   "3ddb820c2f9c722")
...
def _manifest_audit() -> dict:
    ...
    return {
        ...
        "finding": (
            ...
            "byte-identical to slice 64 — so four entries now assert data that "
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join(
        REPO, "artifacts", "slice65_data_freshness.json"))
    ...
    payload = {
        ...
        "delta_since_slice64": {
            ...
            "new_linear_bars_since_slice64": grew_linear,
            "new_funding_prints_since_slice64": grew_funding,
            ...
            "finding": (
                ...
                f"bar(s) since slice 64 and {grew_funding} new funding "
                f"print(s), taking the forward window to {len(closed_after_t1)} "
                f"bars — the second consecutive growth, after 1 (slices 62-63) "
                f"and 2 (slice 64). The ceiling is nevertheless still 0: six "
                f"closed forward bars are needed before one trade can close. "
                f"Three slices have now reported zero forward trades from "
                f"windows of 1, 2 and 3 bars, and that is NOT three failed "
                f"attempts — the ceiling was zero in every one, so the rule "
                f"has not yet had an opportunity to produce a trade. "
                f"EDGE.md §48a, §48b."),
        },
        ...
        "digests": {
            ...
            "note_digest_matches_the_compressed_file":
                gz_sha256(cp.LINEAR_BTC) == NOTE_LINEAR_SHA,
            "slice64_linear_sha256_uncompressed":
                "ab88e2b10f5e97a5cf65ff4a9536fd6d9ecfaaee614a7b16f229e8c6e2b"
                "02aed",
            "linear_identical_to_slice64_uncompressed":
                linear.whole_file_sha256 ==
                ("ab88e2b10f5e97a5cf65ff4a9536fd6d9ecfaaee614a7b16f229e8c6e2b"
                 "02aed"),
            "linear_differs_from_slice64_because_a_bar_arrived": True,
            "the_trap": (
        ...
        "human_note_claims_checked": {
            ...
            "after_t1_dates = ['2026-08-10','2026-08-11','2026-08-12']":
                [c["time_period_start"][:10] for c in closed_after_t1]
                == ["2026-08-10", "2026-08-11", "2026-08-12"],
            "linear rows = 1464": linear.rows_on_disk == 1464,
            "linear sha256 39dcd468... (compressed)":
                gz_sha256(cp.LINEAR_BTC) == NOTE_LINEAR_SHA,
            "append-only; no pre-t1 rewrite": all_append_only,
            "grew vs slice 64 (2 -> 3)": grew_linear == 1,
            "synthetic false": all(
        ...
        "claim_vs_files_discrepancy": not all(_claims.values()),
        "human_note_verdict": (
        ...
        "pack_regression": {
            "recorded_in": "artifacts/slice64_restored_from_slice62.json",
    ...
    delta = payload["delta_since_slice64"]
    print("=" * 78)
    print("SLICE 65 — DATA FRESHNESS   (did it grow SINCE LAST TIME?)")
    print("=" * 78)
    print(f"  t1                          : {folds['t1']}")
    ...
    print(f"    slice 64 uncompressed     : "
          f"{payload['digests']['slice64_linear_sha256_uncompressed']}")
    print(f"    -> identical to slice 64  : "
          f"{payload['digests']['linear_identical_to_slice64_uncompressed']}"
    ...
    print(f"  NEW LINEAR BARS SINCE S64   : "
          f"{delta['new_linear_bars_since_slice64']}")
    print(f"  new funding prints since S64: "
          f"{delta['new_funding_prints_since_slice64']}")
