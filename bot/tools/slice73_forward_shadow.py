...
SLICE 73 — WHAT CHANGED IN THIS FILE, AND WHAT DID NOT
======================================================
...
slice needs to be honest (`window_grew_since_slice72`). `diff` the two files.
...
SCHEMA = "forward_shadow_observation/12"
SLICE = 73
...
def _barrier_eligibility(bars, eligible, closed_forward) -> dict:
    ...
    return {
        ...
        "this_is_what_stopped_an_entry_this_slice": (
            "2026-08-19 is a setup and, since 2026-08-20 closed, a FLAG. It "
            "is not scoreable: no barrier outcome exists for it, because the "
            "corpus ends before its entry bar could resolve. So it never "
            "reaches the schedule, and no entry is taken. The gate is "
            "arithmetic about available data, not a judgement about the "
            "trade."),
...
def build(*, data_dir: str, funding_path: str, warmup: int,
    ...
    return {
        ...
        "ceiling": {
            ...
            "declared_in": "EDGE.md §56a, before this tool ran",
            "observed_forward_closed_trades": forward_n_trades,
            "within_ceiling": forward_n_trades <= ceiling,
            "note_on_this_milestone": (
                "Ceiling 6 at N=11: not 5 — that was slice 72, at N=10 — and "
                "not 7, which would need N=12. What is different this slice "
                "is again upstream of the ceiling: 2026-08-19 is now FLAGGED, "
                "the first forward flag the pilot has produced, because it is "
                "no longer the corpus's last bar. A flag is not an entry. See "
                "state_ladder and barrier_eligibility for the gate that "
                "actually bound. EDGE.md §56b, §56d."),
            ...
            "if_exceeded": (
                "CORRECTED IN SLICE 73 (EDGE.md §55c). Every artefact from "
        ...
        "why_neither_moved": (
            ...
            "and neither does barrier_r_for_all_bars, which is what stopped "
            "an entry here and would have been the most tempting thing in the "
            "tree to adjust. 2026-08-20 is NOT a second setup: its close-join "
            "is 0.00009422, the 16:00 print. The 0.00010000 that printed at "
            "08:00 that day is not the decision rate and quoting it as one "
            "would be the §51c confusion on a single bar."),
        ...
        "window_grew_since_slice72": _window_grew(
            len(closed_forward),
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join(
        REPO, "artifacts", "slice73_forward_shadow.json"))
    parser.add_argument("--log", default=os.path.join(
        REPO, "artifacts", "slice73_forward_shadow.log"))
    ...
    lines = [
        "=" * 78,
        "SLICE 73 — FORWARD SHADOW SEGMENT   (post-t1 bars only)",
        "=" * 78,
        "",
