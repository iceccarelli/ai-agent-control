...
time?"**, and this slice separates the two:
    ...
    new_linear_bars_since_slice71   the delta               (the informative one)
...
PREVIOUS = "artifacts/slice71_data_freshness.json"
...
PRIOR_LINEAR_UNCOMPRESSED = ("59fdf30fc69aad826c9e4e1db08743bcde605d71d39"
                             "05575053a79018be7dbcc")
...
NOTE_LINEAR_SHA = ("a0d0d9139c942012266304fe62f82ee0e1aa14fb8cdd569a"
                   "2e87f8f4598c30ba")
...
NOTE_FUNDING_SHA = ("44c8deda73555ca843d2fe80e3c42bad2ae9b95a6a09ae2f8d3"
                    "7bf6a28c49610")
...
def _pack_base_claim_is_true() -> bool:
    ...
    The decisive test is whether EDGE.md carries the sections slices 62-71
    wrote. The slice-71 deliverable has 45 through 54; a slice-61 base has
...
def _forward_funding_regime() -> dict:
    ...
    return {
        ...
        "newest_decision_is_final": (
            ...
            "EDGE.md §54b."),
        # CORRECTED IN SLICE 72. As written in slice 71 this asked whether
        ...
        # in slice 71 for the wrong reason and went False here while coverage
        # was in fact better. Inverted relative to its name; nothing consumed
        # it, so no measurement was affected. EDGE.md §55h.
        ...
        "finding": (
            f"THE FIRST FORWARD SETUP. "
            f"{sum(1 for i in window if i in setups)} of {len(window)} "
            f"forward decisions reached FUND_ABS = {fb.FUND_ABS}: "
            f"2026-08-19, whose 16:00 print of 0.00010000 stood at the bar's "
            f"close and equals the threshold exactly. The comparison is "
            f"inclusive, so it is a SHORT setup. "
            "THIS IS NOT THE SAME EVENT AS 2026-08-12. That print also "
            "equalled FUND_ABS and was superseded by two later prints before "
            "its bar's close, so it never became a decision rate; §51c has "
            "kept 'a qualifying rate existed that day' and 'the qualifying "
            "rate stood at the decision' apart for four slices, and this is "
            "the first time the second one is true. "
            "IT IS NOT A TRADE AND CANNOT BECOME ONE ON THIS TREE. The rule "
            "enters at next_open and 2026-08-19 is the last bar in the "
            "corpus, so there is no bar to fill on; directed_signal_bars "
            "excludes the final bar for exactly this reason. No flag, no "
            "candidate, no entry, no trade. "
            "NO PARAMETER MOVED TO PRODUCE IT: FUND_ABS is 0.0001 and the "
            "market printed 0.00010000. The threshold did not meet it "
            "halfway. EDGE.md §51c, §55b."),
        ...
        "why_not": (
            ...
            "close-time. Three near misses were recorded across slices 70 "
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join(
        REPO, "artifacts", "slice72_data_freshness.json"))
    ...
    _claims = {
        ...
        "grew vs slice 71 (9 -> 10)": grew_linear == 1,
        "08-20 NOT appended as a linear bar": not any(
            row["time_period_start"].startswith("2026-08-20")
            for row in cp.read_rows(cp.LINEAR_BTC)),
        "ceiling: max(0, 10-5) = 5":
            max(0, len(closed_after_t1) - fb.HORIZON) == 5,
        "funding rows = 4416": funding.rows_on_disk == 4416,
        "funding last print = 2026-08-20T16:00:00+00:00":
        ...
        "08-19 close-join = 0.00010000":
            round(_regime()["rate_at_each_forward_decision"]
                  .get("2026-08-19", 0), 8) == 0.00010000,
        "synthetic false": all(
        ...
        "pack base: tradingbot_slice71.zip": _pack_base_claim_is_true(),
    }
    ...
    payload = {
        ...
        "delta_since_slice71": {
            ...
            "new_linear_bars_since_slice71": grew_linear,
            "new_funding_prints_since_slice71": grew_funding,
        ...
        "digests": {
            ...
            "note_digest_matches_the_compressed_file":
                gz_sha256(cp.LINEAR_BTC) == NOTE_LINEAR_SHA,
            "slice71_linear_sha256_uncompressed": PRIOR_LINEAR_UNCOMPRESSED,
            "linear_identical_to_slice71_uncompressed":
                linear.whole_file_sha256 == PRIOR_LINEAR_UNCOMPRESSED,
            "linear_differs_from_slice71_because_a_bar_arrived": True,

            # The funding half, new this slice and the point of §54d.
            "slice71_funding_sha256_uncompressed": PRIOR_FUNDING_UNCOMPRESSED,
            "funding_identical_to_slice71_uncompressed":
            ...
            "the_trap_did_not_fire_this_slice": (
                ...
                f"through "
                f"{cp.read_rows(cp.FUNDING_BTC)[-1]['funding_time']}. Last "
                f"slice the compressed digest moved while the uncompressed "
                f"stream stood still — recompressed, not changed — and the "
                f"same check is run here so the two situations are told apart "
                f"by the same instrument rather than by the reader's "
                f"expectations. A test that only ever returns one answer is "
                f"not evidence; this one has now returned both. "
                f"EDGE.md §46c, §54d, §55a."),
            "the_trap": (
        ...
        "funding_seam": {
            ...
            "finding": (
                ...
                "again, now for the eleventh slice. EDGE.md §45d."),
        },
        ...
        "claim_vs_files_discrepancy":
            not _claims["grew vs slice 71 (9 -> 10)"],
        "growth_claim_verified": _claims["grew vs slice 71 (9 -> 10)"],
        ...
        "pack_base_claim": {
            "claim": "Pack base: tradingbot_slice71.zip",
            ...
            "why_it_matters": (
                ...
                "has now held for five slices running."),
            ...
            "evidence": (
                f"git show {_BASE_COMMIT[:12]}:EDGE.md — sections 45a-54a all "
        ...
        "what_would_count_as_a_forward_observation": {
            ...
            "ceiling_declared_before_scoring": "EDGE.md §55a",
        },

        "pack_regression": {
            "occurred_this_slice": False,
            "pack_base": "the slice-71 deliverable, verified by digest",
            ...
            "summary": ("No regression. The pack is the previous deliverable, "
                        ...
                        "exists — the fifth consecutive slice with no "
    ...
    delta = payload["delta_since_slice71"]
    print("=" * 78)
    print("SLICE 72 — DATA FRESHNESS   (did it grow SINCE LAST TIME?)")
    print("=" * 78)
    print(f"  t1                          : {folds['t1']}")
    ...
    print(f"    slice 71 uncompressed     : "
          f"{payload['digests']['slice71_linear_sha256_uncompressed']}")
    print(f"    -> identical to slice 71  : "
          f"{payload['digests']['linear_identical_to_slice71_uncompressed']}"
    ...
    print(f"    slice 71 uncompressed     : "
          f"{payload['digests']['slice71_funding_sha256_uncompressed']}")
    print(f"    -> identical to slice 71  : "
          f"{payload['digests']['funding_identical_to_slice71_uncompressed']}"
          f"   (six prints arrived; expected False)")
    print()
    ...
    print(f"  NEW LINEAR BARS SINCE S71   : "
          f"{delta['new_linear_bars_since_slice71']}")
    print(f"  new funding prints since S71: "
          f"{delta['new_funding_prints_since_slice71']}")
