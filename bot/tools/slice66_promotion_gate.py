...
WHY THIS RUN IS DIFFERENT FROM SLICE 65's
=========================================
...
PREVIOUS = "artifacts/slice65_promotion_gate.json"
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join(
        REPO, "artifacts", "slice66_promotion_gate.json"))
    ...
    checklist["forward_shadow_clean"].update({
        ...
        "evidence": (
            ...
            f"— AND THE COUNT DID NOT MOVE, for the fourth consecutive slice. "
            f"6 closed forward bars must exist before the first forward trade "
            f"can close; {closed_bars} do. THIS IS NOT FIVE FAILED ATTEMPTS: "
            f"the ceiling has been zero in every slice from 62 to 66, so the "
            f"rule has not yet had an opportunity to produce a trade. Two more "
            f"closed days take the ceiling to 1, after which a zero would "
            f"carry information. The rule also stood aside on every forward "
            f"bar for want of a funding rate above FUND_ABS. See {FRESHNESS} "
            f"and {FORWARD}."),
    ...
    print("=" * 78)
    print("SLICE 66 — PROMOTION GATE   (re-evaluated against REAL forward data)")
    print("=" * 78)
    ...
    print(f"  items complete               : {payload['items_complete']} / "
          f"{payload['items_total']}   (unchanged from slice 65: "
