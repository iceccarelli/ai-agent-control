...
WHY THIS RUN IS DIFFERENT FROM SLICE 68's
=========================================
...
PREVIOUS = "artifacts/slice68_promotion_gate.json"
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join(
        REPO, "artifacts", "slice69_promotion_gate.json"))
    ...
    checklist["forward_shadow_clean"].update({
        ...
        "evidence": (
            ...
            f"{closed_bars} BARS AND THE CEILING IS NOW 2 — two forward "
            ...
            f"decision is below FUND_ABS. TWO POSSIBLE TRADES DO NOT ADVANCE "
            f"THIS GATE, which asks for 20 completed forward trades and 180 "
            f"forward days; a larger ceiling changes the denominator of what "
            f"is possible, never the numerator of what has happened. See "
    ...
    payload = {
        ...
        "items_complete_unchanged_from_slice_68":
    ...
    print("=" * 78)
    print("SLICE 69 — PROMOTION GATE   (re-evaluated against REAL forward data)")
    print("=" * 78)
    ...
    print(f"  items complete               : {payload['items_complete']} / "
          f"{payload['items_total']}   (unchanged from slice 68: "
          f"{payload['items_complete_unchanged_from_slice_68']})")
