...
WHY THIS RUN IS DIFFERENT FROM SLICE 71's
=========================================
...
PREVIOUS = "artifacts/slice71_promotion_gate.json"
SLICE = 72
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join(
        REPO, "artifacts", "slice72_promotion_gate.json"))
    ...
    checklist["forward_shadow_clean"].update({
        ...
        "evidence": (
            ...
            f"{forward['window_grew_since_slice71']['setups_in_window']} of "
            f"{closed_bars} decisions reached FUND_ABS, and a SETUP IS NOT A "
            f"TRADE — this gate counts trades that CLOSED. "
    ...
    payload = {
        ...
        "items_complete_unchanged_from_slice_71":
    ...
    print("=" * 78)
    print("SLICE 72 — PROMOTION GATE   (re-evaluated against REAL forward data)")
    print("=" * 78)
    ...
    print(f"  items complete               : {payload['items_complete']} / "
          f"{payload['items_total']}   (unchanged from slice 71: "
          f"{payload['items_complete_unchanged_from_slice_71']})")
