...
SLICE 72 — WHAT CHANGED IN THIS FILE, AND WHAT DID NOT
======================================================
...
slice needs to be honest (`window_grew_since_slice71`). `diff` the two files.
...
SCHEMA = "forward_shadow_observation/11"
SLICE = 72
...
def build(*, data_dir: str, funding_path: str, warmup: int,
    ...
    return {
        ...
        "ceiling": {
            ...
            "declared_in": "EDGE.md §55a, before this tool ran",
            "observed_forward_closed_trades": forward_n_trades,
            "within_ceiling": forward_n_trades <= ceiling,
            "note_on_this_milestone": (
                "Ceiling 5 at N=10: not 4 — that was slice 71, at N=9 — and "
                "not 6, which would need N=11. The ceiling is a DENOMINATOR "
                "and it grew because a day closed. What is different this "
                "slice is upstream of it: a decision bar finally reached "
                "FUND_ABS. That changes the SETUP count, not this one, and "
                "not the numerator."),
            ...
            "if_exceeded": (
                "CORRECTED IN SLICE 72 (EDGE.md §55c). Every artefact from "
        ...
        "window_grew_since_slice71": _window_grew(
            len(closed_forward),
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join(
        REPO, "artifacts", "slice72_forward_shadow.json"))
    parser.add_argument("--log", default=os.path.join(
        REPO, "artifacts", "slice72_forward_shadow.log"))
    ...
    lines = [
        "=" * 78,
        "SLICE 72 — FORWARD SHADOW SEGMENT   (post-t1 bars only)",
        "=" * 78,
        "",
