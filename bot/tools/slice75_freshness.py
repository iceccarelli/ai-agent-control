...
time?"**, and this slice separates the two:
    ...
    new_linear_bars_since_slice74   the delta               (the informative one)
...
# Slice 74: sixth. Slice 75: seventh.
NOTE_PATH = f"docs/human/HUMAN_DATA_NOTE_SLICE{SLICE}.md"
PREVIOUS = "artifacts/slice74_data_freshness.json"
...
PRIOR_FUNDING_UNCOMPRESSED = ("3d5c95bff85392397dc8ef6ee6e40176448dec17f3c6"
                              "af7043b80da664f98bc0")
...
# EDGE.md §58f.
NOTE_LINEAR_SHA = ("7fadf2656ca4855c9fab2df11bb549e4fd790c6ee19f5d91"
                   "78fafb5408de21df")
...
NOTE_FUNDING_SHA = ("fec0ee8c33b528cb0baeda4de0c3d5b0c3bb15a144bc5537cd"
                    "379e3bda4219b2")
...
def _pack_base_claim_is_true() -> bool:
    ...
    wrote. The slice-74 deliverable has 45 through 57; a slice-61 base has
...
def _forward_funding_regime() -> dict:
    ...
    return {
        ...
        "finding": (
            ...
            "The newest, 2026-08-22, close-joins at 0.00010000 — its 16:00 "
            "print, which stands at the bar's close (23:59:59.999Z, strictly "
            "before the 2026-08-23T00:00Z print) — and the comparison is "
            "inclusive, so it is a SHORT setup. It is the LAST BAR, so "
            "directed_signal_bars excludes it and it is not a flag. "
            "2026-08-21 made exactly that transition this slice: it was a "
            "last-bar setup in slice 74 and is a FLAG here, as 2026-08-19 was "
            "in slice 73. "
...
def _base_rate_runs() -> dict:
    ...
    All three forward setups are prints at exactly FUND_ABS, which §57c
    established is the venue's BASE rate rather than an extreme. A reader
    seeing three setups arrive in four days could take that for a regime
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join(
        REPO, "artifacts", "slice75_data_freshness.json"))
    ...
    _claims = {
        "closed bars after t1 = 13": len(closed_after_t1) == 13,
        ...
        == ["2026-08-10", "2026-08-11", "2026-08-12", "2026-08-13",
            ...
            "2026-08-22"],
        "linear rows = 1474": linear.rows_on_disk == 1474,
        ...
        "grew vs slice 74 (12 -> 13)": grew_linear == 1,
        "08-23 NOT appended as a linear bar": not any(
            row["time_period_start"].startswith("2026-08-23")
            for row in cp.read_rows(cp.LINEAR_BTC)),
        "ceiling: max(0, 13-5) = 8":
            max(0, len(closed_after_t1) - fb.HORIZON) == 8,
        "funding rows = 4424": funding.rows_on_disk == 4424,
        "funding last print = 2026-08-23T08:00:00+00:00":
        ...
        # The note quotes 08-22's CLOSE-JOIN. Scored against the join, not
        # against a row, so a print and a decision cannot be collapsed.
        # EDGE.md §51c, §58b.
        "08-22 close-join = 0.00010000":
            round(_regime()["rate_at_each_forward_decision"]
                  .get("2026-08-22", 0), 8) == 0.00010000,
        "08-22 is a setup": "2026-08-22" in _regime()["setup_dates"],
        ...
        "pack base: tradingbot_slice74.zip": _pack_base_claim_is_true(),
    }
    ...
    payload = {
        ...
        "delta_since_slice74": {
            ...
            "new_linear_bars_since_slice74": grew_linear,
            "new_funding_prints_since_slice74": grew_funding,
            ...
            "finding": (
                ...
                f"closed trade. EDGE.md §58a, §58b."),
        },
        ...
        "digests": {
            ...
            "note_digest_matches_the_compressed_file":
                gz_sha256(cp.LINEAR_BTC) == NOTE_LINEAR_SHA,
            "slice74_linear_sha256_uncompressed": PRIOR_LINEAR_UNCOMPRESSED,
            "linear_identical_to_slice74_uncompressed":
                linear.whole_file_sha256 == PRIOR_LINEAR_UNCOMPRESSED,
            "linear_differs_from_slice74_because_a_bar_arrived": True,

            # The funding half, new this slice and the point of §54d.
            "slice74_funding_sha256_uncompressed": PRIOR_FUNDING_UNCOMPRESSED,
            "funding_identical_to_slice74_uncompressed":
        ...
        "funding_seam": {
            ...
            "finding": (
                ...
                "again, now for the fourteenth slice. EDGE.md §45d."),
        },
        ...
        "claim_vs_files_discrepancy":
            not _claims["grew vs slice 74 (12 -> 13)"],
        "growth_claim_verified": _claims["grew vs slice 74 (12 -> 13)"],
        ...
        "pack_base_claim": {
            "claim": "Pack base: tradingbot_slice74.zip",
            ...
            "why_it_matters": (
                ...
                "has now held for eight slices running."),
        ...
        "pack_regression": {
            ...
            "pack_base": "the slice-74 deliverable, verified by digest",
            ...
            "summary": ("No regression. The pack is the previous deliverable, "
                        ...
                        "exists — the eighth consecutive slice with no "
    ...
    delta = payload["delta_since_slice74"]
    print("=" * 78)
    print("SLICE 75 — DATA FRESHNESS   (did it grow SINCE LAST TIME?)")
    print("=" * 78)
    print(f"  t1                          : {folds['t1']}")
    ...
    print(f"    slice 74 uncompressed     : "
          f"{payload['digests']['slice74_linear_sha256_uncompressed']}")
    print(f"    -> identical to slice 74  : "
          f"{payload['digests']['linear_identical_to_slice74_uncompressed']}"
    ...
    print(f"    slice 74 uncompressed     : "
          f"{payload['digests']['slice74_funding_sha256_uncompressed']}")
    print(f"    -> identical to slice 74  : "
          f"{payload['digests']['funding_identical_to_slice74_uncompressed']}"
    ...
    print(f"  NEW LINEAR BARS SINCE S74   : "
          f"{delta['new_linear_bars_since_slice74']}")
    print(f"  new funding prints since S74: "
          f"{delta['new_funding_prints_since_slice74']}")
