...
SLICE 70 — WHAT CHANGED IN THIS FILE, AND WHAT DID NOT
======================================================
...
slice needs to be honest (`window_grew_since_slice69`). `diff` the two files.
...
def build(*, data_dir: str, funding_path: str, warmup: int,
    ...
    return {
        ...
        "ceiling": {
            ...
            "declared_in": "EDGE.md §53b, before this tool ran",
            "observed_forward_closed_trades": forward_n_trades,
            "within_ceiling": forward_n_trades <= ceiling,
            "note_on_this_milestone": (
                "Ceiling 3 at N=8: not 2 — that was slice 69, at N=7 — and "
                "not 4, which would need N=9. The ceiling is a DENOMINATOR: "
        ...
        "join_rule": ("last funding print at-or-before the bar's CLOSE — "
                      ...
                      "backwards in time"),
        "why_neither_moved": (
            "TWO near misses now exist and neither is rescued. (1) The "
            ...
            "change is even relevant — only a lower FUND_ABS would capture "
            "it, by 8e-6 (EDGE.md §53c). Between them they exhaust the two "
            "available rescues, which is exactly why neither is taken: "
            "adjusting a threshold or a join AFTER seeing which prints missed "
            "is fitting the rule to the data, and the rule was cleared under "
            "these values. The adjustment that would rescue either bar is "
            "precisely the one that would void the clear."),
        ...
        "window_grew_since_slice69": _window_grew(len(closed_forward)),
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join(
        REPO, "artifacts", "slice70_forward_shadow.json"))
    parser.add_argument("--log", default=os.path.join(
        REPO, "artifacts", "slice70_forward_shadow.log"))
    ...
    lines = [
        "=" * 78,
        "SLICE 70 — FORWARD SHADOW SEGMENT   (post-t1 bars only)",
        "=" * 78,
        "",
