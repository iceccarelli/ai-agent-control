...
time?"**, and this slice separates the two:
    ...
    new_linear_bars_since_slice62   the delta               (the informative one)
...
=================================================
`HUMAN_DATA_NOTE_SLICE63.md` quotes a linear sha256 that differs from slice
...
SCHEMA = "data_freshness/4"
NOTE_PATH = "docs/human/HUMAN_DATA_NOTE_SLICE63.md"
PREVIOUS = "artifacts/slice62_data_freshness.json"
NOTE_LINEAR_SHA = ("db05f3f8b4ff6f5fd4f54dc5d2286504f5486b125972d1b6e73e3b690"
                   "f30225d")
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join(
        REPO, "artifacts", "slice63_data_freshness.json"))
    ...
    payload = {
        ...
        "delta_since_slice62": {
            ...
            "new_linear_bars_since_slice62": grew_linear,
            "new_funding_prints_since_slice62": grew_funding,
            ...
            "why_this_field_exists": (
                ...
                "EDGE.md §46c."),
            "finding": (
                f"The linear corpus did NOT grow: {grew_linear} new closed "
                f"daily bars since slice 62. Exactly {grew_funding} funding "
                f"print arrived. THE DECISION CLOCK IS THE DAILY BAR — a "
                f"funding print without a bar to decide is not an observation, "
                f"a rule slice 61's tool already put in writing. This slice "
                f"therefore measures the SAME WINDOW slice 62 measured."),
        },
        ...
        "digests": {
            ...
            "slice62_linear_sha256_uncompressed":
                "32ca5971229f07ed866627d939ed07768f7e0e0d299a3d4c07d440c0b43"
                "bab22",
            "linear_identical_to_slice62_uncompressed":
        ...
        "human_note_verdict": (
            "Every claim in the note is INDEPENDENTLY TRUE. The note does not "
            "claim growth since slice 62 and there was none, so nothing it "
            "says is contradicted. Its sha256 is of the COMPRESSED file, which "
            "is not the digest any check in this repository uses — "
            "corroboration, not the check."),
    ...
    delta = payload["delta_since_slice62"]
    print("=" * 78)
    print("SLICE 63 — DATA FRESHNESS   (did it grow SINCE LAST TIME?)")
    print("=" * 78)
    print(f"  t1                          : {folds['t1']}")
    ...
    print(f"    slice 62 uncompressed     : "
          f"{payload['digests']['slice62_linear_sha256_uncompressed']}")
    print(f"    -> identical to slice 62  : "
          f"{payload['digests']['linear_identical_to_slice62_uncompressed']}")
    print()
    ...
    print(f"  NEW LINEAR BARS SINCE S62   : "
          f"{delta['new_linear_bars_since_slice62']}")
    print(f"  new funding prints since S62: "
          f"{delta['new_funding_prints_since_slice62']}")
