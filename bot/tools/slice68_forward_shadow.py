...
SLICE 68 — WHAT CHANGED IN THIS FILE, AND WHAT DID NOT
======================================================
...
slice needs to be honest (`window_grew_since_slice67`). `diff` the two files.
...
SCHEMA = "forward_shadow_observation/8"
...
def _window_grew(closed_forward_bars: int) -> dict:
    ...
    return {
        ...
        "note": (
            "THE WINDOW REACHED SIX BARS AND THE CEILING IS 1 — the first "
            "structural non-zero value since t1 was locked. EDGE.md §49b "
            "predicted this from a four-bar window; §50c confirmed its first "
            "half at five bars; this is the second half. A completed forward "
            "trade is now STRUCTURALLY PERMITTED, which removes an obstacle "
            "and supplies no evidence. Three conditions must hold and only the "
            "first has changed: (1) enough closed bars — now true; (2) a "
            "funding setup fires at |f| >= FUND_ABS — NOT true, zero setups on "
            "all six bars; (3) the frozen one_entry_per_contiguous_run "
            "schedule — unchanged. The six preceding zeros carried no "
            "information about the edge because the rule had no opportunity. "
            "THIS ONE IS DIFFERENT AND NARROWLY SO: it says the funding regime "
            "over 2026-08-10..15 produced no qualifying rate at any decision. "
            "It says NOTHING about whether the rule makes money, because no "
            "position was opened — only its selectivity was exercised, not its "
            "skill. The cleared run scheduled 41 trades in two years, about "
            "one per eighteen days, so six quiet days is unremarkable under "
            "the null and the alternative alike. EDGE.md §51b, §51d."),
    }
...
def build(*, data_dir: str, funding_path: str, warmup: int,
    ...
    return {
        ...
        "ceiling": {
            ...
            "declared_in": "EDGE.md §51b, before this tool ran",
        ...
        "window_grew_since_slice67": _window_grew(len(closed_forward)),
        ...
        "why_not_closer": (
            "One post-t1 day is pilot progress, not autonomy. The gate counts "
            "closed forward trades and it counts "
            f"{forward_n_trades}."),
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join(
        REPO, "artifacts", "slice68_forward_shadow.json"))
    parser.add_argument("--log", default=os.path.join(
        REPO, "artifacts", "slice68_forward_shadow.log"))
    ...
    lines = [
        "=" * 78,
        "SLICE 68 — FORWARD SHADOW SEGMENT   (post-t1 bars only)",
        "=" * 78,
        "",
        ...
        "",
        "-" * 78,
        "  THE CEILING, DECLARED BEFORE THIS RUN (EDGE.md §51b)",
        "-" * 78,
        ...
        "-" * 78,
        f"  constants f'print            : "
        f"{payload['constants_fingerprint']}  (matches slice 58: "
