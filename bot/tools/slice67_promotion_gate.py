...
WHY THIS RUN IS DIFFERENT FROM SLICE 66's
=========================================
...
PREVIOUS = "artifacts/slice66_promotion_gate.json"
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join(
        REPO, "artifacts", "slice67_promotion_gate.json"))
    ...
    checklist["forward_shadow_clean"].update({
        ...
        "evidence": (
            ...
            f"trades; 0 of {pg.MIN_FORWARD_DAYS} days. THE WINDOW GREW AGAIN — "
            f"{closed_bars} closed linear daily bars now lie strictly after t1 "
            f"— AND THE COUNT DID NOT MOVE, for the fifth consecutive slice. "
            f"6 closed forward bars must exist before the first forward trade "
            f"can close; {closed_bars} do. THIS IS NOT SIX FAILED ATTEMPTS: "
            f"the ceiling has been zero in every slice from 62 to 67, so the "
            f"rule has not yet had an opportunity to produce a trade, and no "
            f"quiet day here proves or disproves the clear. ONE more closed "
            f"day takes the ceiling to 1, after which a zero would carry "
            f"information. Separately and unchanged: the rule has not "
            f"signalled on any forward bar, no funding rate having reached "
            f"FUND_ABS. See {FRESHNESS} and {FORWARD}."),
    ...
    payload = {
        ...
        "items_complete_unchanged_from_slice_63":
    ...
    print("=" * 78)
    print("SLICE 67 — PROMOTION GATE   (re-evaluated against REAL forward data)")
    print("=" * 78)
    ...
    print(f"  items complete               : {payload['items_complete']} / "
          f"{payload['items_total']}   (unchanged from slice 66: "
          f"{payload['items_complete_unchanged_from_slice_63']})")
