...
WHY THIS RUN IS DIFFERENT FROM SLICE 64's
=========================================
...
PREVIOUS = "artifacts/slice64_promotion_gate.json"
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join(
        REPO, "artifacts", "slice65_promotion_gate.json"))
    ...
    checklist["forward_shadow_clean"].update({
        ...
        "evidence": (
            ...
            f"('2026-08-10', '2026-08-11', '2026-08-12') — AND THE COUNT DID "
            f"NOT MOVE, for the third consecutive slice. With a 5-bar horizon "
            f"and next-open entry, 6 closed forward bars must exist before the "
            f"first forward trade can close; {closed_bars} do. THIS IS NOT "
            f"THREE FAILED ATTEMPTS: the ceiling has been zero in all three, "
            f"so the rule has not yet had an opportunity to produce a trade. "
            f"The rule also stood aside on every bar for want of a funding "
            f"setup above FUND_ABS. See {FRESHNESS} and {FORWARD}."),
    ...
    print("=" * 78)
    print("SLICE 65 — PROMOTION GATE   (re-evaluated against REAL forward data)")
    print("=" * 78)
    ...
    print(f"  items complete               : {payload['items_complete']} / "
          f"{payload['items_total']}   (unchanged from slice 64: "
