...
SLICE 75 — WHAT CHANGED IN THIS FILE, AND WHAT DID NOT
======================================================
...
slice needs to be honest (`window_grew_since_slice74`). `diff` the two files.
...
SCHEMA = "forward_shadow_observation/14"
SLICE = 75
...
def build(*, data_dir: str, funding_path: str, warmup: int,
    ...
    return {
        ...
        "ceiling": {
            ...
            "declared_in": "EDGE.md §58a, before this tool ran",
            "observed_forward_closed_trades": forward_n_trades,
            "within_ceiling": forward_n_trades <= ceiling,
            "note_on_this_milestone": (
                "Ceiling 8 at N=13: not 7 — that was slice 74, at N=12 — and "
                "not 9, which would need N=14. Two things moved upstream: "
                "2026-08-21 became a FLAG, the transition 2026-08-19 made in "
                "slice 73, and 2026-08-22 is a THIRD setup on the last bar. "
                "Nothing is scoreable: eligibility reaches back seven bars, "
                "so the highest scoreable bar is 2026-08-15. The gaps are "
                "recomputed from the corpus every slice and never carried "
                "forward — §57b reused a distance and got it wrong. "
                "EDGE.md §58b, §57e, §56d."),
        ...
        "window_grew_since_slice74": _window_grew(
            len(closed_forward),
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join(
        REPO, "artifacts", "slice75_forward_shadow.json"))
    parser.add_argument("--log", default=os.path.join(
        REPO, "artifacts", "slice75_forward_shadow.log"))
    ...
    lines = [
        "=" * 78,
        "SLICE 75 — FORWARD SHADOW SEGMENT   (post-t1 bars only)",
        "=" * 78,
        "",
