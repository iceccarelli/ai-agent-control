...
time?"**, and this slice separates the two:
    ...
    new_linear_bars_since_slice72   the delta               (the informative one)
...
PREVIOUS = "artifacts/slice72_data_freshness.json"
...
PRIOR_LINEAR_UNCOMPRESSED = ("67c6f88ed078729f2c181c86fd0fa879e955e26a888"
                             "41e5e3e2e078f0e557d59")
...
PRIOR_FUNDING_UNCOMPRESSED = ("44c8deda73555ca843d2fe80e3c42bad2ae9b95a6a09"
                              "ae2f8d37bf6a28c49610")
...
NOTE_LINEAR_SHA = ("17086a985f0e2bb49c8323fcb697e1562415ec58883d2f86"
                   "59b19f95dc2ed44e")
...
NOTE_FUNDING_SHA = ("96a8db5a8e29cba7ad61361704c8d196b0c34aa035f9fe4089c"
                    "2e44a52f928f3")
...
def _pack_base_claim_is_true() -> bool:
    ...
    wrote. The slice-72 deliverable has 45 through 55; a slice-61 base has
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join(
        REPO, "artifacts", "slice73_data_freshness.json"))
    ...
    _claims = {
        ...
        "grew vs slice 72 (10 -> 11)": grew_linear == 1,
        "08-21 NOT appended as a linear bar": not any(
            row["time_period_start"].startswith("2026-08-21")
            for row in cp.read_rows(cp.LINEAR_BTC)),
        "ceiling: max(0, 11-5) = 6":
            max(0, len(closed_after_t1) - fb.HORIZON) == 6,
        "funding rows = 4419": funding.rows_on_disk == 4419,
        "funding last print = 2026-08-21T16:00:00+00:00":
        ...
        # The note quotes 08-20's CLOSE-JOIN, which is the 16:00 print, not
        # the 0.00010000 that printed at 08:00. Scored against the join so
        # the two cannot be collapsed. EDGE.md §51c, §56b.
        "08-20 close-join = 0.00009422":
            round(_regime()["rate_at_each_forward_decision"]
                  .get("2026-08-20", 0), 8) == 0.00009422,
        "synthetic false": all(
        ...
        "pack base: tradingbot_slice72.zip": _pack_base_claim_is_true(),
    }
    ...
    payload = {
        ...
        "delta_since_slice72": {
            ...
            "new_linear_bars_since_slice72": grew_linear,
            "new_funding_prints_since_slice72": grew_funding,
            ...
            "finding": (
                ...
                f"closed trade. EDGE.md §56a, §56b."),
        },
        ...
        "digests": {
            ...
            "note_digest_matches_the_compressed_file":
                gz_sha256(cp.LINEAR_BTC) == NOTE_LINEAR_SHA,
            "slice72_linear_sha256_uncompressed": PRIOR_LINEAR_UNCOMPRESSED,
            "linear_identical_to_slice72_uncompressed":
                linear.whole_file_sha256 == PRIOR_LINEAR_UNCOMPRESSED,
            "linear_differs_from_slice72_because_a_bar_arrived": True,

            # The funding half, new this slice and the point of §54d.
            "slice72_funding_sha256_uncompressed": PRIOR_FUNDING_UNCOMPRESSED,
            "funding_identical_to_slice72_uncompressed":
        ...
        "funding_seam": {
            ...
            "finding": (
                ...
                "again, now for the twelfth slice. EDGE.md §45d."),
        },
        ...
        "claim_vs_files_discrepancy":
            not _claims["grew vs slice 72 (10 -> 11)"],
        "growth_claim_verified": _claims["grew vs slice 72 (10 -> 11)"],
        ...
        "pack_base_claim": {
            "claim": "Pack base: tradingbot_slice72.zip",
            ...
            "why_it_matters": (
                ...
                "has now held for six slices running."),
            ...
            "evidence": (
                f"git show {_BASE_COMMIT[:12]}:EDGE.md — sections 45a-55a all "
        ...
        "what_would_count_as_a_forward_observation": {
            ...
            "ceiling_declared_before_scoring": "EDGE.md §56a",
        },

        "pack_regression": {
            "occurred_this_slice": False,
            "pack_base": "the slice-72 deliverable, verified by digest",
            ...
            "summary": ("No regression. The pack is the previous deliverable, "
                        ...
                        "exists — the sixth consecutive slice with no "
    ...
    delta = payload["delta_since_slice72"]
    print("=" * 78)
    print("SLICE 73 — DATA FRESHNESS   (did it grow SINCE LAST TIME?)")
    print("=" * 78)
    print(f"  t1                          : {folds['t1']}")
    ...
    print(f"    slice 72 uncompressed     : "
          f"{payload['digests']['slice72_linear_sha256_uncompressed']}")
    print(f"    -> identical to slice 72  : "
          f"{payload['digests']['linear_identical_to_slice72_uncompressed']}"
    ...
    print(f"    slice 72 uncompressed     : "
          f"{payload['digests']['slice72_funding_sha256_uncompressed']}")
    print(f"    -> identical to slice 72  : "
          f"{payload['digests']['funding_identical_to_slice72_uncompressed']}"
    ...
    print(f"  NEW LINEAR BARS SINCE S72   : "
          f"{delta['new_linear_bars_since_slice72']}")
    print(f"  new funding prints since S72: "
          f"{delta['new_funding_prints_since_slice72']}")
