...
WHY THIS RUN IS DIFFERENT FROM SLICE 67's
=========================================
...
PREVIOUS = "artifacts/slice67_promotion_gate.json"
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join(
        REPO, "artifacts", "slice68_promotion_gate.json"))
    ...
    checklist["forward_shadow_clean"].update({
        ...
        "evidence": (
            ...
            f"{closed_bars} BARS AND THE CEILING IS NOW 1 — the first "
            f"structural non-zero value since t1 was locked, and the first "
            f"slice in which a completed forward trade was permitted at all. "
            f"NO SETUP FIRED: every funding rate at a decision was below "
            f"FUND_ABS, so the count is still zero. ONE POSSIBLE TRADE DOES "
            f"NOT ADVANCE THIS GATE, which asks for 20 completed forward "
            f"trades and 180 forward days; a ceiling of 1 changes the "
            f"denominator of what is possible, not the numerator of what has "
            f"happened. See {FRESHNESS} and {FORWARD}."),
    ...
    print("=" * 78)
    print("SLICE 68 — PROMOTION GATE   (re-evaluated against REAL forward data)")
    print("=" * 78)
    ...
    print(f"  items complete               : {payload['items_complete']} / "
          f"{payload['items_total']}   (unchanged from slice 67: "
