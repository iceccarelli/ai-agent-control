...
SLICE 65 — WHAT CHANGED IN THIS FILE, AND WHAT DID NOT
======================================================
...
slice needs to be honest (`window_grew_since_slice64`). `diff` the two files.
...
SCHEMA = "forward_shadow_observation/5"
...
def _window_grew(closed_forward_bars: int) -> dict:
    ...
    return {
        "comparable": True,
        "previous_artefact": "artifacts/slice64_forward_shadow.json",
        ...
        "note": (
            "THE WINDOW GREW AGAIN — 2 bars to 3, the third consecutive "
            "measurement and the second consecutive growth. The ceiling did "
            "not move: max(0, 3 - HORIZON) is still zero. Three slices have "
            "now reported forward_n_trades = 0 from windows of 1, 2 and 3 "
            "bars, and that sequence is NOT three failed attempts — the "
            "ceiling was zero in every one of them, so the rule has not yet "
            "been given an opportunity to produce a trade at all. The earliest "
            "slice at which one becomes arithmetically possible is the one "
            "with SIX closed forward bars. EDGE.md §48b."),
    }
...
def build(*, data_dir: str, funding_path: str, warmup: int,
    ...
    return {
        ...
        "window_grew_since_slice64": _window_grew(len(closed_forward)),
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join(
        REPO, "artifacts", "slice65_forward_shadow.json"))
    parser.add_argument("--log", default=os.path.join(
        REPO, "artifacts", "slice65_forward_shadow.log"))
    ...
    lines = [
        "=" * 78,
        "SLICE 65 — FORWARD SHADOW SEGMENT   (post-t1 bars only)",
        "=" * 78,
        "",
