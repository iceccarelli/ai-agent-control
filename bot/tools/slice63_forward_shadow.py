...
SLICE 63 — WHAT CHANGED IN THIS FILE, AND WHAT DID NOT
======================================================
...
slice needs to be honest (`window_grew_since_slice62`). `diff` the two files.
...
WHAT SLICE 63 MEASURES
======================
The same window slice 62 measured. `|W| = 1`, unchanged: the linear corpus is
byte-identical to slice 62's (§46c) and only a funding print arrived. The
decision clock is the daily bar, so the forward window did not grow, and the
honest description of this slice's zero is not "zero again" as though something
were tried and failed — it is *there is nothing new to measure, and the
apparatus says so.*
...
SCHEMA = "forward_shadow_observation/3"
...
def _window_grew(closed_forward_bars: int) -> dict:
    ...
    return {
        ...
        "note": (
            "The linear corpus is byte-identical to slice 62's; only a funding "
            "print arrived, and the decision clock is the daily bar. This "
            "slice therefore measures the SAME WINDOW. The zero below is not a "
            "failed attempt — there is nothing new to attempt. EDGE.md §46c."),
    }
...
def build(*, data_dir: str, funding_path: str, warmup: int,
    ...
    return {
        ...
        "ceiling": {
            ...
            "declared_in": "EDGE.md §45e, before this tool ran",
        ...
        "window_grew_since_slice62": _window_grew(len(closed_forward)),
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join(
        REPO, "artifacts", "slice63_forward_shadow.json"))
    parser.add_argument("--log", default=os.path.join(
        REPO, "artifacts", "slice63_forward_shadow.log"))
    ...
    lines = [
        "=" * 78,
        "SLICE 63 — FORWARD SHADOW SEGMENT   (post-t1 bars only)",
        "=" * 78,
        "",
