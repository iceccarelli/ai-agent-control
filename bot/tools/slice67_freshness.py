...
time?"**, and this slice separates the two:
    ...
    new_linear_bars_since_slice66   the delta               (the informative one)
...
SCHEMA = "data_freshness/8"
NOTE_PATH = "docs/human/HUMAN_DATA_NOTE_SLICE64.md"
PREVIOUS = "artifacts/slice66_data_freshness.json"
NOTE_LINEAR_SHA = ("5c15e8ff01e264a806ef87e4fb06dff95122539111addfce"
                   "dc3967c1a30dc666")
...
def _pack_base_claim_is_true() -> bool:
    """Is the note's claim about its own PACK BASE true? EDGE.md §50a.

    The note says "Pack base: post-restore slice66 tree (not slice-61)". The
    decisive test is whether EDGE.md contains the sections slices 62-66 wrote:
    a slice-66 base has §45 through §49, a slice-61 base has none of them.

    Checked here rather than asserted in prose because this is the first claim
    a human note has made about PROCESS rather than about data — and a human
    can verify a bar count by looking at the file they wrote, but cannot verify
    which parent their build script used by looking at anything.
    """
    with open(os.path.join(REPO, "EDGE.md"), encoding="utf-8") as handle:
        edge = handle.read()
    # As DELIVERED, before this slice restored anything. The restore artefact
    # records what was missing; this reads that record rather than today's file.
    restored = os.path.join(REPO, "artifacts",
                            "slice67_restored_from_slice66.json")
    if os.path.isfile(restored):
        with open(restored, encoding="utf-8") as handle:
            evidence = json.load(handle).get(
                "evidence_the_pack_is_the_slice61_tree", {})
        if evidence:
            return False
    return all(f"\n## {n}a." in edge for n in (45, 46, 47, 48, 49))
...
def _manifest_audit() -> dict:
    ...
    return {
        ...
        "finding": (
            ...
            "byte-identical to slice 66 — so four entries now assert data that "
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join(
        REPO, "artifacts", "slice67_data_freshness.json"))
    ...
    _claims = {
        "closed bars after t1 = 5": len(closed_after_t1) == 5,
        "after_t1_dates": [c["time_period_start"][:10]
                           for c in closed_after_t1]
        == ["2026-08-10", "2026-08-11", "2026-08-12", "2026-08-13",
            "2026-08-14"],
        "linear rows = 1466": linear.rows_on_disk == 1466,
        "linear sha256 (compressed)":
            gz_sha256(cp.LINEAR_BTC) == NOTE_LINEAR_SHA,
        "append-only; no pre-t1 rewrite": all_append_only,
        "grew vs slice 67 (4 -> 5)": grew_linear == 1,
        "ceiling still 0 (need 6)":
            max(0, len(closed_after_t1) - fb.HORIZON) == 0,
        "synthetic false": all(
        ...
        # THE ONE THAT FAILS. Recorded in the same dict as the rest, on the
        # same terms, so a false claim cannot be quarantined out of the score.
        "pack base: post-restore slice66 tree (not slice-61)":
            _pack_base_claim_is_true(),
    }
    ...
    payload = {
        ...
        "delta_since_slice66": {
            ...
            "new_linear_bars_since_slice66": grew_linear,
            "new_funding_prints_since_slice66": grew_funding,
            ...
            "why_this_field_exists": (
                ...
                "EDGE.md §46c."),
            "finding": (
                f"THE WINDOW GREW AGAIN. {grew_linear} new closed daily "
                f"bar(s) since slice 66 and {grew_funding} new funding "
                f"print(s), taking the forward window to {len(closed_after_t1)} "
                f"bars (1,1,2,3,4,5 across slices 62-67). The ceiling is still "
                f"0 because max(0, 5 - HORIZON) = 0, and this is the LAST "
                f"slice at which zero follows from the window being shorter "
                f"than the horizon. EDGE.md §49b predicted exactly this before "
                f"the data existed; its other half — |W| = 6 gives ceiling 1 — "
                f"is one closed day away. EDGE.md §50c."),
        },
        ...
        "digests": {
            ...
            "note_digest_matches_the_compressed_file":
                gz_sha256(cp.LINEAR_BTC) == NOTE_LINEAR_SHA,
            "slice66_linear_sha256_uncompressed":
                "86089882a7fa305e4a37ce4ab0348ef11efc363f09af27234d2f88ad2a5"
                "a1dfe",
            "linear_identical_to_slice66_uncompressed":
                linear.whole_file_sha256 ==
                ("86089882a7fa305e4a37ce4ab0348ef11efc363f09af27234d2f88ad2a5"
                 "a1dfe"),
            "linear_differs_from_slice66_because_a_bar_arrived": True,
            "the_trap": (
        ...
        "claim_vs_files_discrepancy": not _claims["grew vs slice 67 (4 -> 5)"],
        "growth_claim_verified": _claims["grew vs slice 67 (4 -> 5)"],
        ...
        "pack_base_claim": {
            "claim": "Pack base: post-restore slice66 tree (not slice-61)",
            ...
            "evidence": "artifacts/slice67_restored_from_slice66.json",
        ...
        "pack_regression": {
            "recorded_in": "artifacts/slice66_restored_from_slice62.json",
    ...
    delta = payload["delta_since_slice66"]
    print("=" * 78)
    print("SLICE 67 — DATA FRESHNESS   (did it grow SINCE LAST TIME?)")
    print("=" * 78)
    print(f"  t1                          : {folds['t1']}")
    ...
    print(f"    slice 66 uncompressed     : "
          f"{payload['digests']['slice66_linear_sha256_uncompressed']}")
    print(f"    -> identical to slice 66  : "
          f"{payload['digests']['linear_identical_to_slice66_uncompressed']}"
    ...
    print(f"  NEW LINEAR BARS SINCE S66   : "
          f"{delta['new_linear_bars_since_slice66']}")
    print(f"  new funding prints since S66: "
          f"{delta['new_funding_prints_since_slice66']}")
