...
SLICE 67 — WHAT CHANGED IN THIS FILE, AND WHAT DID NOT
======================================================
...
slice needs to be honest (`window_grew_since_slice66`). `diff` the two files.
...
SCHEMA = "forward_shadow_observation/7"
...
def _window_grew(closed_forward_bars: int) -> dict:
    ...
    return {
        "comparable": True,
        "previous_artefact": "artifacts/slice66_forward_shadow.json",
        ...
        "note": (
            "THE WINDOW GREW AGAIN — 4 bars to 5. The ceiling is still zero: "
            "max(0, 5 - HORIZON) = 0. THIS IS THE LAST SLICE AT WHICH ZERO "
            "FOLLOWS FROM THE WINDOW BEING SHORTER THAN THE HORIZON. EDGE.md "
            "§49b predicted this before the data existed — |W| = 4 -> 0, "
            "|W| = 5 -> 0, |W| = 6 -> 1 — and the first half has now come "
            "true. Six slices have reported forward_n_trades = 0 and EVERY ONE "
            "ran at ceiling 0, so none of them is evidence for or against the "
            "edge; the rule has never had an opportunity. One more closed day "
            "changes that. A second condition is independent of the window and "
            "has not changed in five slices: the rule has not SIGNALLED — no "
            "forward funding rate has reached FUND_ABS = 1e-4, and the newest "
            "print (6.29e-06) is the smallest yet. At |W| = 6 a trade needs "
            "the funding regime to change, not merely the window to lengthen. "
            "EDGE.md §50c, §50d."),
    }
...
def build(*, data_dir: str, funding_path: str, warmup: int,
    ...
    return {
        ...
        "ceiling": {
            ...
            "declared_in": "EDGE.md §50c, before this tool ran",
            "observed_forward_closed_trades": forward_n_trades,
            "within_ceiling": forward_n_trades <= ceiling,
            "note_on_the_next_milestone": (
                "N = 6 is the first structurally non-zero ceiling. It is NOT "
                "claimed this slice: at N = 5 the arithmetic gives 0."),
            "if_exceeded": (
        ...
        "window_grew_since_slice66": _window_grew(len(closed_forward)),
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join(
        REPO, "artifacts", "slice67_forward_shadow.json"))
    parser.add_argument("--log", default=os.path.join(
        REPO, "artifacts", "slice67_forward_shadow.log"))
    ...
    lines = [
        "=" * 78,
        "SLICE 67 — FORWARD SHADOW SEGMENT   (post-t1 bars only)",
        "=" * 78,
        "",
        ...
        "",
        "-" * 78,
        "  THE CEILING, DECLARED BEFORE THIS RUN (EDGE.md §50c)",
        "-" * 78,
